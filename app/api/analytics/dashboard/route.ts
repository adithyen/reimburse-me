  ...
  try {
    ...
    const peopleSummaryData = (peopleSummary || [])
      .map((p) => ({
        ...
        totalOutstanding: (p.debtRecords || []).reduce((s, d) => s + (d.outstandingAmount || 0), 0),
      }))
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        pendingDebtCount: debtSummary?._count?.id || 0,
        ...
        oldestDebt: oldestDebt
          ? {
              id: oldestDebt.id,
              person: oldestDebt.person?.name || 'Contact',
              amount: oldestDebt.outstandingAmount || 0,
              daysOld: oldestDebt.createdAt
                ? Math.floor((now.getTime() - new Date(oldestDebt.createdAt).getTime()) / 86400000)
                : 0,
            }
          : null,
      },
